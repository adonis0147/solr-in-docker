...
SOLR_BIN='/home/hadoop/solr-current/bin'
...
function start_solr() {
  log_info "start solr"
  while read -r host; do
    if ! "${SOLR_BIN}/solr" zk ls /solr -z "${host}:2181"; then
      if ! "${SOLR_BIN}/solr" zk mkroot /solr -z "${host}:2181"; then
        log_error 'failed to mkroot /solr in zookeeper'
        return 1
      fi
    fi
    ssh -n "${host}" "${SOLR_BIN}/solr start -cloud"
  done < "${SERVERS}"
  return 0
}
